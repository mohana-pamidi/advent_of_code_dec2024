...
bool acutallySafe(std::vector<int>* levels)
{
    if(checkReportSafe(levels))
    {
        return true;
    }
    ...
    {
        std::vector<int> leftSide(levels->begin(), levels->begin() + i);

        std::vector<int> rightSide(levels->begin() + i + 1, levels->end());

        std::vector<int> merged = leftSide;

        merged.insert(merged.end(), rightSide.begin(), rightSide.end());

        if(checkReportSafe(&merged))
        {
            return true;
        }
    }
